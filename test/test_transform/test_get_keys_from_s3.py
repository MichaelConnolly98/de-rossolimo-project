from src.transform.pandas_testing import get_keys_from_s3
import pytest
import boto3
...
def test_get_s3_gets_file_with_prefix_from_existing_bucket(s3_client):
    ...
    result = get_keys_from_s3("file", bucket_name="test-bucket")
    ...
    result_2 = get_keys_from_s3("file", bucket_name="test-bucket")
    assert result_2 == ['table=file/path.json', "table=file/path2.json"]
...
def test_get_s3_raises_error_on_incorrect_prefix(s3_client, caplog):
    ...
    with pytest.raises(KeyError):
        get_keys_from_s3("nonsense", bucket_name="test-bucket")
