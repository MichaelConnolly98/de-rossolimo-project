from src.transform.dim_payment import payment_dim
...
def test_payment_dim_returns_data_frame():
    response_df = payment_dim(dataframes['payment_type'])
    assert type(response_df) == pd.core.frame.DataFrame

def test_payment_dim_returns_correct_columns():
    response_df = payment_dim(dataframes['payment_type'])
...
def test_payment_dim_returns_rows_with_correct_data_types():
    response_df = payment_dim(dataframes['payment_type'])
