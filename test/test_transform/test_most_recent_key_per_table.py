from utils.most_recent_pandas import get_most_recent_key_per_table_from_s3_single
import pytest
import boto3
...
def test_get_s3_raises_error_on_incorrect_table_name(s3_client, caplog):
    with pytest.raises(ClientError):
        get_most_recent_key_per_table_from_s3_single("nonsense")
...
def test_get_s3_gets_file_with_prefix_from_existing_bucket(s3_client):
    ...
    result = get_most_recent_key_per_table_from_s3_single(
    ...
    result_2 = get_most_recent_key_per_table_from_s3_single(
        "file", bucket_name="test-bucket"
    )
...
def test_get_s3_raises_error_on_incorrect_prefix(s3_client, caplog):
    ...
    with pytest.raises(KeyError):
        get_most_recent_key_per_table_from_s3_single(
