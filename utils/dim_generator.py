...
def counterparty_dim(file_dict=None):
    ...
    if isinstance(part_counterparty_df, pd.DataFrame):
        ...
        desired_columns_and_order = [
            "counterparty_legal_name", "address_line_1", "address_line_2",
...
def transaction_dim(file_dict=None):
    ...
    if isinstance(transaction_df, pd.DataFrame):
        ...
        transaction_df["sales_order_id"] = transaction_df[
            "sales_order_id"].astype('Int64')
        transaction_df["sales_order_id"] = transaction_df[
            "sales_order_id"].astype(object).where(pd.notnull(
                transaction_df["sales_order_id"]), None)
        transaction_df["purchase_order_id"] = transaction_df[
            "purchase_order_id"].astype('Int64')
        transaction_df["purchase_order_id"] = transaction_df[
            "purchase_order_id"].astype(object).where(pd.notnull(
                transaction_df["purchase_order_id"]), None)
