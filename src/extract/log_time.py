...
def get_timestamp_from_logs(log_group_name='/aws/lambda/extract-de_rossolimo'):
    ...
    try:
        ...
        result = datetime.fromtimestamp(last_event_time/1000).strftime('%Y-%m-%d %H:%M:%S')
        ...
        logger.error({"result": "Failure", "Error": "log_group_name parameter must be a valid log group"})
        return {"Result": "Failure", "Error": InvalidInput}

    except Exception as e:
        logger.error({"result": "Failure", "Error": f"Exception has occured: {repr(e)}"})
